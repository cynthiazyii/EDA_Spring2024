...
```{r table,echo=FALSE}
table1<-data.frame(matrix(c("Source","Date","Filename","EPA Air Quality System (AQS)",
                            "2018-2019", "EPAair_O3_PM25_NC1819_Processed.csv"),
                          nrow=3,ncol=2))
colnames(table1) <- c("Item","Value")
knitr::kable(table1,align =c("r","l"))
```
...
knitr::kable(summary_PM25,caption = "Table 2: Mean Particulates (2.5mm)",col.names = c("County","\u00B5g/m\u00B3"))
knitr::kable(summary_Ozone,caption = "Table 3: Mean Ozone",col.names = c("County","\u00B5g/m\u00B3"))
...
```{r bp1,echo=FALSE, fig.cap="2018 Ozone values by month",warning=FALSE, fig.align='left',fig.height=4}
data.2018<-epa_data %>% 
  filter(Year == "2018")

ggplot(data.2018,aes(x=Month,y=Ozone,group=Month))+
  geom_boxplot()+
  labs(title = "Monthly Ozone concentrations - 2018",x= "Month", y="Concentration (\u00B5g/m\u00B3)")+
  scale_x_continuous(breaks =1:12 ,
                     labels = c("Jan", "Feb", "Mar", "Apr","May","Jun","Jul","Agu","Sep","Oct","Nov","Dec"))

```

```{r bp2,echo=FALSE, fig.cap="2019 Ozone values by month",warning=FALSE, fig.align='left',fig.height=4}
data.2019<-epa_data %>% 
  filter(Year == "2019")

ggplot(data.2019,aes(x=Month,y=Ozone,group=Month))+
  geom_boxplot()+
...
https://github.com/cynthiazyii/EDA_Spring2024
